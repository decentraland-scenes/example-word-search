...
export class Creator {
  ...
  public words: string[] = [];
  public wordsCoords: {start: Coordinate, direction: Direction, length: number}[] = [];
  ...
  */
  ...
  public addWordsToGrid() {
    ...
    for (i = 0; i < this.getWordCount(); i++) {
      ...
      while (!added && attempts < Creator.maxAttempts) {
        added = this.addWordToGrid(this.getWord(i), i);
        attempts++;
      }
  ...
  public addWordToGrid(word: string, wordIndex: number) {
